...
"""This is the main entry-point for the experiments with the Minecraft environment."""
import logging
import os
import shutil
...
from MyMinecraft.game.minecraft_env import MinecraftConfiguration, ActionSpaceType
from pythomata.dfa import DFA

from MyMinecraft.env import make_env_from_dfa, make_env
...
def parse_args():
    parser = ArgumentParser("minecraft")
    parser.add_argument("--goal-reward", type=int, default=10, help="The reward for satisfying a single task.")
    parser.add_argument("--seed", type=int, default=42, help="Random seed.")
    parser.add_argument("--output-dir", type=str, default="experiments/MyMinecraft-output", help="Output directory for the experiment results.")
    parser.add_argument("--overwrite", action="store_true", default=False, help="Overwrite the content of the output directory.")
    parser.add_argument("--expert-config", type=str, default="MyMinecraft/expert_config.yaml", help="RL configuration for the expert.")
    parser.add_argument("--learner-config", type=str, default="MyMinecraft/learner_config.yaml", help="RL configuration for the learner.")
...
def run_expert(arguments, configuration):
    ...
    config = MinecraftConfiguration(horizon=100, nb_goals=nb_tasks, action_space_type=ActionSpaceType.NORMAL)
    env = make_env(config, arguments.output_dir, arguments.goal_reward, reward_shaping=configuration.reward_shaping)

    np.random.seed(arguments.seed)
    env.seed(arguments.seed)

    policy = AutomataPolicy(tuple(-idx - 2 for idx in range(config.nb_goals)),
                            nb_steps=configuration.nb_exploration_steps, value_max=1.0, value_min=configuration.min_eps)
